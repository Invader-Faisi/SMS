...
class extends Component {
    ...
    public function mount(\App\Services\StudentManagementServices $studentServices, string $id): void
    {
        $this->students = $studentServices->getStudentsByClass($id);
        $this->class = $id;
    }
...
<section class="p-2 space-y-2">
    @if(!$students->isEmpty())
        ...
        @else
            <!-- danger Alert -->
            <div class="relative w-full overflow-hidden rounded-md border border-red-500 bg-surface text-on-surface dark:bg-surface-dark dark:text-on-surface-dark" role="alert">
